...
from: aristo-mini: https://github.com/allenai/aristo-mini
"""
...
class MultipleChoiceQuestion(NamedTuple):
    ...
    def from_jsonl(line: str) -> 'MultipleChoiceQuestion':
        ...
        return MultipleChoiceQuestion(
            id_=blob['id'],
            stem=question['stem'],
            choices=[Choice(c["label"], c["text"]) for c in
                     question['choices']],
...
def parse_question(blob: Dict[str, Any]) -> MultipleChoiceQuestion:
    """parses a question from a json blob. is possibly too lenient to
    malformed json """
    return MultipleChoiceQuestion(
        stem=blob.get("stem", ""),
        choices=[Choice(c["label"], c["text"]) for c in
